...
public class createMedicalRecordsDaoImpl implements createMedicalRecordsDao {
    ...
    public boolean createMedicalRecord(MedicalRecords medicalRecords) {
        try (Connection con = ConnectionHelper.getConnection()) {
            ...
            Integer nurseId = medicalRecords.getNurseInChargeId();
    ...
    public MedicalRecords findMedicalInformationByLRN(long LRN) {
        ...
        try (Connection con = ConnectionHelper.getConnection()) {
            String sql = "SELECT " +
                    ...
                    "n.first_name || ' ' || n.middle_name || ' ' || n.last_name AS nurse_in_charge_name " +
                    "FROM student s " +
                    "JOIN person p ON s.person_id = p.id " +
                    "LEFT JOIN medical_record mr ON s.id = mr.student_id " +
                    "LEFT JOIN person n ON mr.nurse_in_charge_id = n.id " +
                    "WHERE s.LRN = ?";
            ...
            try (PreparedStatement stmt = con.prepareStatement(sql)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        medicalRecords.setNurseInChargeId(rs.getInt("nurse_in_charge_id"));
                        medicalRecords.setNurseInChargeName(rs.getString("nurse_in_charge_name"));
                    }
                }
            }
        } catch (SQLException e) {
